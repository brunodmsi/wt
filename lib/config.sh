...
# Usage: generate_default_config "project_name" "repo_root" "config_file"
...
# ============================================================================
# This file was auto-generated by wt. Customize it to match your project.
# Full reference: https://github.com/brunodmsi/wt/blob/main/docs/configuration.md
...
# Port allocation
# ============================================================================
# Reserved ports use slot-based allocation so each worktree gets unique ports.
# Formula: port = range.min + (slot * number_of_services) + service_offset
#
# With the defaults below (min=3000, 3 slots, 1 service at offset 0):
#   Slot 0 → port 3000
#   Slot 1 → port 3001
#   Slot 2 → port 3002
#
# Increase slots if you need more concurrent worktrees.
# Adjust the range to avoid conflicts with other tools.
ports:
  reserved:
    range: { min: 3000, max: 3009 }
    slots: 3
    services:
      app: 0       # offset 0 -customize this to match your service names
      # api: 1     # add more services with increasing offsets
  dynamic:
    range: { min: 4000, max: 5000 }
    services: {}
    # worker: true  # services here get hash-based ports (same branch = same port)

# ============================================================================
# Environment variables
# ============================================================================
# Exported before setup steps, services, and hooks run.
# Use \${PORT_<SERVICE>} to reference allocated ports (e.g. \${PORT_APP}).
env:
  NODE_ENV: development

# ============================================================================
# Setup steps -run automatically on 'wt create'
# ============================================================================
# Each step runs in the worktree directory. Steps run in order unless
# 'depends_on' is specified for dependency-based execution.
#
# Fields:
#   name         -unique identifier (required)
#   command      -shell command to run (required)
#   working_dir  -relative to worktree root (default: ".")
#   description  -human-readable label shown during execution
#   on_failure   -abort (default) | continue | retry
#   depends_on   -list of step names that must complete first
#   condition    -shell expression; step is skipped when it evaluates to false
#   env          -extra KEY: VALUE pairs for this step only
setup:
  - name: install-deps
    description: "Install project dependencies"
    command: echo "TODO: replace with your install command (e.g. npm install)"
    working_dir: "."
    on_failure: abort
  # - name: build
  ...
  #   working_dir: "."
  #   depends_on: [install-deps]
  #   condition: "[ -f prisma/schema.prisma ]"

# ============================================================================
# Services -managed by 'wt start' / 'wt stop'
# ============================================================================
# Each service runs in its own tmux pane. Use port_key to bind a service
# to a reserved port slot (the name must match a key under ports.reserved.services).
#
# Fields:
#   name        -unique identifier (required)
#   command     -shell command to start the service (required)
#   working_dir -relative to worktree root (default: ".")
#   port_key    -links to a ports.reserved.services entry for port allocation
#   env         -extra KEY: VALUE pairs; \${PORT} is the allocated port
#   health_check:
#     type      -tcp | http
#     url       -for http checks (e.g. http://localhost:\${PORT}/health)
#     timeout   -max seconds to wait (default: 30)
#     interval  -seconds between checks (default: 2)
services:
  - name: app
    description: "Main application server"
    command: echo "TODO: replace with your start command (e.g. npm run dev)"
    working_dir: "."
    port_key: app
    env:
      PORT: "\${PORT}"
...
# tmux layout
# ============================================================================
# Controls how panes are arranged when you 'wt create' or 'wt attach'.
# Layouts: tiled | even-horizontal | even-vertical
#
# Pane types:
#   service: <name>   -runs and monitors the named service
#   command: "<cmd>"   -runs a one-off command (empty string = interactive shell)
tmux:
  layout: tiled
  windows:
    - name: dev
...
# Lifecycle hooks
# ============================================================================
# Shell snippets that run at specific points in the worktree lifecycle.
# Available variables: \${BRANCH_NAME}, \${WORKTREE_PATH}, \${PORT_<SERVICE>}
#
# Hooks:
#   pre_create   -before the worktree directory is created
#   post_create  -after worktree + setup complete
#   pre_start    -before services are started
#   post_start   -after all services are running
#   post_stop    -after services are stopped
#   pre_delete   -before the worktree is removed
#   post_delete  -after cleanup is done
hooks: {}
  # post_create: |
...
# When in a git repo with no config, auto-generates a default configuration.
...
require_project() {
    ...
    if [[ -z "$project" ]]; then
        ...
        if [[ -z "$project" ]]; then
            # Try auto-init if we're in a git repo
            project=$(auto_init_project)
            if [[ -z "$project" ]]; then
                die "$error_msg"
            fi
        fi
...
# Auto-initialize a project configuration when in a git repo without config.
# Returns project name on success, empty string on failure.
auto_init_project() {
    ...
    # If inside a worktree subdirectory, resolve to main repo
    ...
    log_info "No configuration found. Auto-generating default config for '$project_name'..."
    ...
    log_info "Customize it with: wt config --edit -p $project_name"
